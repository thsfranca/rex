...
- **Step caps:** Removed (**R069** / [ADR 0034](../../docs/architecture/decisions/0034-remove-tool-step-caps.md)). `tool_steps` metrics still increment **only** when `should_bill_tool_step` is true: any `ok=True` or exploratory broker failure bills; **policy/config** failures (daemon `mode_denied`, `access policy denied`, sidecar validation) do **not** bill — see [ADR 0013](../../docs/architecture/decisions/0013-access-policy-broker-completion.md).
