...
- Route tab completion to fastest model, chat/agent to default strongest model
...
- ML classifier training:
  ...
  - Per-cluster model performance tracking (latency, token count, retry rate)
- CLI tool (`rex stats`):
...
- The router automatically adjusts which model serves each discovered category based on tracked outcomes
