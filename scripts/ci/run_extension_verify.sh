...
if [ "${result}" = "success" ]; then
  if ! SKIP_VSCE_PREPUBLISH=1 npm run package:vsix 2>&1 | tee "${artifacts_dir}/package.log"; then
    mark_failure "TestExecution" "PACKAGE_FAIL" "Run npm run build && SKIP_VSCE_PREPUBLISH=1 npm run package:vsix locally."
  fi
fi
echo "::endgroup::"
