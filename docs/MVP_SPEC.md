...
- **Daemon-supervised sidecars** (process + optional OS sandbox + broker; **not** VM-default on Mac): design hubs [SIDECAR_RUNTIME.md](SIDECAR_RUNTIME.md), [AGENT_ACCESS_POLICY.md](AGENT_ACCESS_POLICY.md); deferred VM catalog [AGENT_RUNTIME_ENVIRONMENT.md](AGENT_RUNTIME_ENVIRONMENT.md); not required for Phase 1 acceptance.
