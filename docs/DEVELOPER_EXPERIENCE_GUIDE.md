...
|---|---|
| `rex` | Unified CLI: `daemon`, `status`, `complete`, `tui` (**R073** shipped; design system **R082**; implement **R080–R081**). |
...
rex tui  # interactive terminal harness (R073); design system R082
```

**Current focus:** Implement TUI design system — **R080** → **R081** — [TUI_DESIGN.md](TUI_DESIGN.md), [CLI_OPERATOR_UX.md](CLI_OPERATOR_UX.md), [TERMINAL_HARNESS_ARCHITECTURE.md](TERMINAL_HARNESS_ARCHITECTURE.md). Auto-start **R071** shipped. **`rex status`** / **`rex complete`** auto-start the daemon by default; opt out with **`daemon.auto_start: false`** or **`--no-daemon-autostart`**. Install with **`./scripts/install-cli.sh`**. Agent live validation of **`rex tui`**: tuiwright MCP text snapshots — see [TUI_DESIGN.md](TUI_DESIGN.md#validation).
