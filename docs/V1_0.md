...
|----|-----------|------|-------------------|--------|
| **RC-01** | `rex.v1` and NDJSON contract stable; breaking changes only via documented major migration | Contract | Conformance fixtures; [CI.md](CI.md); DX guide §5 | **Partial** |
| **RC-02** | Operator path documented and script-verified: clone → HTTP backend → daemon+sidecar → extension chat | Customer-visible | [EXTENSION_LOCAL_E2E.md](EXTENSION_LOCAL_E2E.md); [scripts/verify_mvp_local.sh](../scripts/verify_mvp_local.sh); [MVP_SPEC.md](MVP_SPEC.md) operator verification | **Partial** |
...
| **RC-04** | Brokered HTTP inference for sidecar requests | Capability | [ADAPTERS.md](ADAPTERS.md); [CONFIGURATION.md](CONFIGURATION.md); `broker.rs` / sidecar turn | **Met** |
| **RC-05** | Centralized **AccessPolicy broker** evaluates all sidecar tool requests before host execution; structured deny | Capability | [POLICY_ENGINE.md](POLICY_ENGINE.md); [AGENT_ACCESS_POLICY.md](AGENT_ACCESS_POLICY.md); tests | **Not met** |
...
| **RC-08** | Actionable failures for missing daemon, sidecar, HTTP backend, or `rex-cli` on PATH | Quality | CLI/extension messages; tests | **Partial** |
| **RC-09** | Economics floor: implemented matrix rows stable; routing logs **`route=`** and a **decision id** (beyond env-only hook) | Quality | [CONTEXT_EFFICIENCY.md](CONTEXT_EFFICIENCY.md); daemon logs/tests | **Partial** |
