...
| oMLX managed local inference (Mac) | Daemon-supervised oMLX; OpenAI-compat wire | `http_openai_compat` + `inference.omlx.*` + provider profile `omlx` | [OMLX_INFERENCE.md](OMLX_INFERENCE.md), [ADR 0033](architecture/decisions/0033-omlx-managed-local-inference.md) | **planned** |
