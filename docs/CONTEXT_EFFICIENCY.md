...
| oMLX managed local inference (Mac) | Daemon-supervised oMLX; OpenAI-compat wire | `http_openai_compat` + `inference.omlx.*` injects `openai_compat.base_url` | [OMLX_INFERENCE.md](OMLX_INFERENCE.md), [ADR 0033](architecture/decisions/0033-omlx-managed-local-inference.md) | **implemented** |
...
| Bounded FTS5 session memory (desktop / daemon) | Retrieve capped fragments per turn; avoid unbounded context stuffing | Daemon SQLite in **`.rex/`**; desktop stateless | [historical/TERMINAL_HARNESS_ARCHITECTURE.md](historical/TERMINAL_HARNESS_ARCHITECTURE.md), **R076** | **design accepted** |
