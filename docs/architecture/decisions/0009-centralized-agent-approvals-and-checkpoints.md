...
1. **Multiple clients, one safety surface.** `rex-cli`, the rex CLI, and any future scripted client all reach the same `rex.v1` daemon. Approval logic that lives only in one client cannot enforce safety for the others ([`docs/ROADMAP.md`](../../ROADMAP.md) explicitly excludes "unattended multi-file coordinated agent runs without user approvals" because the thin extension cannot replace daemon-side tool policy).
...
2. **Extension still owns approval UX.** Capturing user intent (prompt, granted permissions, "approve this step" buttons) and surfacing approval-required states stays in [`rex CLI`](../../../) per [`docs/NDJSON_STREAM.md`](../../NDJSON_STREAM.md). The extension communicates approval context to the daemon; the daemon authorizes.
...
- [`docs/NDJSON_STREAM.md`](../../NDJSON_STREAM.md) — extension owns the approval UX surface.
