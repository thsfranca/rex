...
1. **Default product path:** `daemon.socket_scope: per_workspace` (implicit when unset). Derive daemon and host sidecar UDS paths from a stable hash of canonical process **cwd** under `$REX_ROOT/sockets/`.
2. **Legacy escape hatch:** `daemon.socket_scope: global` uses explicit `daemon.socket` (single daemon for all clients).
3. **CLI ensure:** resolve workspace from canonical **cwd**; autostart with `current_dir` = that path; per-socket autostart lock.
