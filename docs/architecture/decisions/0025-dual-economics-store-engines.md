...
Deep research (2026-06-07) evaluated embedded SQL, cloud TSDBs, and columnar layouts. [ADR 0027](0027-chce-columnar-mmap-engine.md) adopts **CHCE** (columnar-mmap engine) for the mmap path; physical layout is in [OBS_STORE_MMAP_FORMAT.md](../../historical/OBS_STORE_MMAP_FORMAT.md).
...
1. **`rex-obs-store` exposes one logical write API** with two engines selected by merged JSON:
 ...
 - **`observability.store.engine: "mmap"`** (opt-in, **macOS only**) — append-only mmap file per [OBS_STORE_MMAP_FORMAT.md](../../historical/OBS_STORE_MMAP_FORMAT.md) (default path `obs/store.rexobs`).
...
6. **Default engine promotion** — Remains **`sqlite`** until promotion gates in [OBS_STORE_MMAP_FORMAT.md](../../historical/OBS_STORE_MMAP_FORMAT.md) §Promotion gates are met; then a follow-up ADR amendment may flip the JSON default.
...
- **Related:** Dual-engine policy lives here; CHCE on-disk detail in [OBS_STORE_MMAP_FORMAT.md](../../historical/OBS_STORE_MMAP_FORMAT.md) and [ADR 0027](0027-chce-columnar-mmap-engine.md).
...
- [OBS_STORE_MMAP_FORMAT.md](../../historical/OBS_STORE_MMAP_FORMAT.md) · [OBSERVABILITY_AND_ECONOMICS.md](../../historical/OBSERVABILITY_AND_ECONOMICS.md) · [CONFIGURATION.md](../../CONFIGURATION.md)
