...
|----------|--------|-------|-----------|
| **Generative outputs** (LLM → tools) | Provider-native function calling + **structured outputs** (`strict: true`) | Phase 2 | **R038** — [NATIVE_TOOL_CALLING.md](../../NATIVE_TOOL_CALLING.md) |
| **Interim generative** (until R038; fallback retained) | One-line JSON `{"type":"tool"\|"final",...}` in text | Phase 1 | Current (**R018**) |
...
**Phase 1 trigger:** keep JSON-in-text + raw delimited tool results until **R038** ships native `tools[]` on direct Ollama (default); interim path retained for mock CI and gateway edge cases — [NATIVE_TOOL_CALLING.md](../../NATIVE_TOOL_CALLING.md).
...
- [ADR 0016](0016-mcp-in-sidecar-envelope.md) — MCP Phase 2 with **R033** (rescoped; follows R038)
