...
Research proposed `REX_TEST_MODE` for deterministic animation. Rex config policy ([KISS config surface](../../CONFIGURATION.md)) limits product env vars to **`REX_ROOT`**; probe behavior already pins sessions via **`REX_HARNESS_SESSION_ID`** ([harness_session.rs](../../../crates/rex-cli/src/harness_session.rs)).
...
4. **Harness determinism:** When `REX_HARNESS_SESSION_ID` matches the probe fixture, use **SteppedClock** — forced Idle tier + PTY control sequences to advance animation time. No new cosmetic env var.
