...
**Three tiers:** (1) **Per-PR** — **`mock`** / harness paths in `uds_e2e` and a **loopback OpenAI-compat HTTP fixture** in `mvp_product_path` (real `http_openai_compat`, no cloud API) — **RC-10**. (2) **Planned opt-in + nightly** — live Ollama smoke (**R039**, **R040**) — [ECONOMICS_VALIDATION.md](ECONOMICS_VALIDATION.md). (3) **Operator dogfood** — opt-in native tool E2E [`verify_native_tools_live.sh`](../scripts/verify_native_tools_live.sh) (`REX_LIVE_LLM=1`; not in `ci-checks`) plus manual live JSON `inference.openai_compat` (Ollama, LM Studio, etc.) — [EXTENSION_LOCAL_E2E.md](EXTENSION_LOCAL_E2E.md).
