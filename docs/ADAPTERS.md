...
|---|---|
| `native_tool_calling` | Forward OpenAI `tools[]` / `tool_choice`; parse SSE `delta.tool_calls`; Ollama `/api/show` probe — **R038** — [NATIVE_TOOL_CALLING.md](NATIVE_TOOL_CALLING.md). Default broker profile for agent tools: **direct Ollama** (`http://127.0.0.1:11434/v1`); gateway opt-in. |
