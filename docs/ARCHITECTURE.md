...
REX is a local AI runtime: `rex-daemon` owns streaming, policy, caches, and context shaping; thin clients (Electron desktop, automation scripts) use gRPC over UDS. The product path is a supervised sidecar agent with brokered HTTP inference. Primary operator surface: **Electron desktop app** ([ADR 0043](architecture/decisions/0043-electron-shell-for-electric-alive-compositor.md), [WEB_UI_ARCHITECTURE.md](WEB_UI_ARCHITECTURE.md)).
