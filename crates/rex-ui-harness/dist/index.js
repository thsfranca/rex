...
const server = new Server({ name: "rex-ui-harness", version: "0.1.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "ui_open",
            description: "Launch Rex desktop session (Electron + production dist + mock daemon).",
            inputSchema: {
                type: "object",
                properties: {
                    mode: {
...
async function handleTool(name, args) {
    switch (name) {
        ...
        case "ui_snapshot": {
            ...
            const payload = { tree, mode: s.mode };
            if (args.screenshot) {
        ...
        case "ui_assert_layout": {
            ...
            const checks = {};
            if (args.display)
                checks.display = layout.display === args.display;
        ...
        case "ui_wait_for": {
            ...
            if (args.selector)
                await pageWaitForSelector(s, String(args.selector), timeout);
            if (args.text)
                await pageWaitForText(s, String(args.text), timeout);
            return { ok: true };
        }
